...
def check_cv_match(model, cv_text):
    ...
    print('model.predict([cv_text]) ===> ', model.predict([cv_text]))
...
def extract_standards_from_text(text):
    ...
    standards = [ent.text for ent in doc.ents if ent.label_ == "Education:"]
    return standards
...
def checkAndCreate():
    ...
    # Define a pipeline with TfidfVectorizer and Multinomial Naive Bayes classifier
    ...
    model_filename = "./core/AI Models/cv_standards_model.joblib"
    ...
    result = check_cv_match(model, new_text)

    if result:
        return "The provided CV matches the major and contains all sections."
    else:
        return "The provided CV does not match the major or some sections are missing."


print('checkAndCreate() ===> ', checkAndCreate())
